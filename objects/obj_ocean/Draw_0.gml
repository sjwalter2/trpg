...
for(var i = 0; i < width+3; i++)
{
	for(var j = 0; j < height+3; j++)
	{
		...
		draw_roundrect(gridX[i, j],gridY[i, j],gridX[i+1, j+1],gridY[i+1, j+1],0)
		draw_set_color(c_blue)		
		draw_roundrect(gridX[i, j] - gridW/2,gridY[i, j]- gridH/2,gridX[i+1, j+1]- gridW/2,gridY[i+1, j+1]- gridH/2,0)
